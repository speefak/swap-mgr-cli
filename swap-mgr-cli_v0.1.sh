...
show_swap_info () {											# TODO show floating vars 
		...
		HParser="numfmt --to iec --round=nearest --field=2 "
...
clear_swap () {
	...
	SwapSizeLockedVAR=$SwapSize											# fix swap var, cause swapoff changes var

	# start swapoff, check for running swapp off process
	if [[ -z $(ps -aux  | grep -w "swapoff $SwapPath$") ]]; then
		SwapoffPIDFile=/tmp/.smc_cs_pid_$RANDOM
		CSStartTime=$(date +%s)
		touch $SwapoffPIDFile
		screen -dmS "SMC-swapoff" bash -c "swapoff $SwapPath 2> $SwapoffPIDFile && rm $SwapoffPIDFile "		# does not work in background without screen: CMD 2> errorlog &
	...
	while [[ -f $SwapoffPIDFile ]];do
		...
		# print message
		progressbar  "$LYellow clear swap $Reset        $SwapPath | $($HParser <<< $(($SwapSize*1024)))  " 1 "$SwapClearedPercent%"

		# check swapoff output
		if [[ -n $(cat $SwapoffPIDFile | grep "swapoff failed")  ]]; then
			echo											# show last processing line 
	...
	# calculate swapoff time
	RuntimeINT=$(($CSEndTime-$CSStartTime))

	# disable shot time output
	if [[ ! $RuntimeINT == 1 ]]; then Runtime="| $RuntimeINT sec " ; fi

	# enable swap and update swap info
	swapon $SwapPath
	get_swap_info

	printf "$LGreen swap cleared $Reset   $SwapPath | $($HParser <<< $(($SwapSize*1024))) ($SwapUsedPercent%%) $Runtime                                      "
	printf "\n" 
