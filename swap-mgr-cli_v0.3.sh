...
SwapFilePath=/home/
SwapFileName="smc-rd-$(date +%s)"
SwapFileSize="1GB" #numfmt --from iec 1,5G
...
# Benutzer nach dem Pfad für die Swap-Datei fragen
read -p "Geben Sie den Pfad für die Swap-Datei ein (z.B. /pfad/zur/swapdatei): " swap_path

# Benutzer nach der Größe der Swap-Datei fragen (in MB)
read -p "Geben Sie die Größe der Swap-Datei in MB ein: " swap_size_mb

# Pfad zur Swap-Datei erstellen
swap_file="$swap_path/swapfile"

# Größe in Kilobytes umrechnen
swap_size_kb=$((swap_size_mb * 1024))

# Swap-Datei erstellen
sudo dd if=/dev/zero of="$swap_file" bs=1M count="$swap_size_mb"
sudo chmod 600 "$swap_file"  # Berechtigungen setzen
sudo mkswap "$swap_file"     # Swap-Datei formatieren
sudo swapon "$swap_file"     # Swap aktivieren

# Aktuelle Swap-Konfiguration anzeigen
echo "Aktuelle Swap-Konfiguration:"
swapon --show

echo "Swap-Datei wurde erstellt und aktiviert."
...
create_ramdisk() {
    # Prompt the user for the size of the RAM disk in megabytes
    read -p "Enter the size of the RAM disk in megabytes: " ramdisk_size_mb

    # Calculate the size in kilobytes
    ramdisk_size_kb=$((ramdisk_size_mb * 1024))

    # Prompt the user for the mount point of the RAM disk
    read -p "Enter the mount point for the RAM disk: " ramdisk_mount_point

    # Create the RAM disk
    sudo mkdir -p "$ramdisk_mount_point"
    sudo mount -t tmpfs -o size="$ramdisk_size_kb"k tmpfs "$ramdisk_mount_point"
...
#sudo chown -R  speefak.speefak /mnt/ramdisk
    ...
    echo "RAM disk created at $ramdisk_mount_point with size $ramdisk_size_mb MB."
}

# Menu for selecting actions
PS3="Select an option:"
options=("Create RAM Disk" "Quit")
